...
export class RabbitMQLib {
    ...
    static async connectQueue(retryCount = 2, retryDelay = 5000): Promise<void> {
        while (retryCount > 0) {
            try {
    ...
    static async dequeue(processMessage: (message: string) => Promise<void>): Promise<void> {
        if (!this.channel) {
            throw new Error("RabbitMQ channel is not initialized.");
        }

        await this.channel.consume("messages", async (msg) => {
            if (msg) {
